...
class MPTForCausalLM(MPTPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # print(labels[0],end="")
            # print(logits[0],end="")
            labels[labels==50280] = -100 # <PAD> --> -100
            ...
            # print(_labels[0].to(logits.device).view(-1),end="")
            # print(len(_labels[0].to(logits.device).view(-1)))
            # print(logits[0].view(-1, logits.size(-1)),end="")
            # print(logits[0].view(-1, logits.size(-1)).shape)
            # print("333333333333333333333333333")
            # _, predicted_class = torch.max(logits[0].view(-1, logits.size(-1)), 1)
            # print(predicted_class)
            # print(logits.shape)
            # print(_labels[labels!=-100])
